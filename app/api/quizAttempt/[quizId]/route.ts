...
export async function POST(req: NextRequest, context: any) {
  try {
    ...
    for (const attemptKey in attempts) {
      ...
      const isCorrect =
        attempt.length === questions[attemptKey].correctOption.length &&
        attempt.every((answer: any) =>
          questions[attemptKey].correctOption.includes(answer)
        ) &&
        questions[attemptKey].correctOption.every((option: any) =>
          attempt.includes(option)
        );
      if (isCorrect) {
        score++;
      }
    ...
    const result = {
      ...res,
      QuestionAttempt: res.QuestionAttempt.map((questionAttempt) => {
        return {
          ...questionAttempt,
    ...
    if (maxScoreRecord._max.score == null) {
      increment = score;
      newAttempt = true;
    } else if (maxScoreRecord._max.score < score)
      increment = score - maxScoreRecord._max.score;
