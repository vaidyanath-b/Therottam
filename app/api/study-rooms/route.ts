...
try{
  ...
  const memberships = await prisma.studyRoomMember.findMany({
   ...
   include: {
    ...
    const studyRooms : StudyRoomListData[] = memberships.map(
      (membership) => {
        return{
  ...
  return NextResponse.json({ message: "Error fetching study rooms" }, { status: 500 });
}
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ message: "Error creating study room" }, { status: 500 });
  }
}
