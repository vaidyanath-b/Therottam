...
export const PUT = async (req: NextRequest, context: any) => {
  try {
    ...
    const tagIdsToCreate = newTagIds.filter(
      (tagId: string) => !oldTagIds.includes(tagId)
    );

    const tagIdsToDelete = oldTagIds.filter(
      (tagId: string) => !newTagIds.includes(tagId)
    );
    // Create new tags
    const questionTopics = tagIdsToCreate.map((tagId: any) =>
      prisma.questionTopic.create({
        data: {
          questionId: updatedQuestion.id,
          topicId: tagId,
        },
    ...
    const questionTopicsToDelete = tagIdsToDelete.map((tagId: any) =>
      prisma.questionTopic.deleteMany({
        where: {
          questionId: updatedQuestion.id,
          topicId: tagId,
        },
    ...
    if (transactionList.length > 0) await prisma.$transaction(questionTopics);
