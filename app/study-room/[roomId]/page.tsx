  ...
  useEffect(() => {
    async function getDayItems(){
      ...
      .then((data) => {
        ...
        setContents(data.StudyRoomContent)
      })
    }
    getDayItems()
  }, [selectedDay])
...
{            
(!quiz.isLive && !quiz.isDead && quiz.creator_id == userName)? <Button className="h-7" onClick={()=>window.location.href=`/quiz/${quiz.quizId}`}> Edit Quiz 
              </Button>
              :
              quiz.isLive ? quiz.creator_id == userName ?
                           <Button className="h-7" onClick={()=>window.location.href=`/quiz/${quiz.quizId}`}>Edit Quiz</Button> :  //change needed cannot edit while ongoing except for day1 or do something for day1
